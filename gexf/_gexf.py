...
def msg_unexpected_tag(expected, got):
    msg = "Error: incorrect xml. Expected tag {}, not {}."
...
class Graph:
    ...
    def addNode(self, id, label, start="", end="", startopen=False, endopen=False, pid="", r="", g="", b="", spells=[]):
        self._nodes[str(id)] = Node(
            self, id, label, start, end, pid, r, g, b, spells, startopen, endopen)
    ...
    def addEdge(self, id, source, target, weight="", start="", end="", label="", r="", g="", b="", spells=[], startopen=False, endopen=False):
        self._edges[str(id)] = Edge(self, id, source, target, weight,
                                    start, end, label, r, g, b, spells, startopen, endopen)
...
#             raise Exception("attribute id unknown. Add Attribute to graph first")
    ...
    def importXML(graph_xml, gexf_obj):
        """import graph xml tag to create a Graph Object and delegate Node/Edges
        extraction to Edge/Node class"""
...
class Attributes(dict):
    ...
    def __init__(self):
        self.type_choices = [
            "integer", "string", "float", "double", "boolean", "date", "URI"]
    ...
    def declareAttribute(self, attClass, type, defaultValue, title="", mode="static", id=None):
        ...
        if attClass in self.attClass_choices:
            ...
            msg = "wrong attClass : " + str(attClass) + " Should be in " + str(type_choices)
            raise Exception(msg)
    ...
    def makeAttributeInstance(self, attClass, id=None, value=None, start=None, end=None, startopen=False, endopen=False):
        ...
        if attClass in self.attClass_choices:
            if id in self[attClass].keys():
                att = {"id": id}
                att["value"] = value if value else self[
                    attClass][id]["defaultValue"]
                if self[attClass][id]["mode"] == "dynamic" and start or end:
                    # start & end will be discarded if the mode is set to
                    # static
                    if start:
                        att["start"] = start
                    if startopen:
                ...
                msg = "wrong attribute id (%s)" % id
                msg += " declare the attribute first with declareAttribute"
                raise Exception(msg)
        else:
            msg = "wrong attClass: {}. Should be in {}"
            msg = msg % (attClass, self.type_choices)
            raise Exception(msg)
    ...
    def getAttributesDeclarationXML(self):
        ...
        if len(self) > 0:
            ...
            for attClass, atts in self.iteritems():
                ...
                attributes = sorted(atts.values(), key=key_mode, reverse=True)
                mode_to_attributes = groupby(attributes, key_mode)
                for mode, atts in mode_to_attributes:
                    # generate on attributes by mode
                    attributesXML = Element("attributes")
                    ...
                    for att in sorted(atts, key=lambda att: att["id"]):
                        attributeXML = SubElement(attributesXML, "attribute")
                        attributeXML.set("id", str(att["id"]))
                        attributeXML.set("title", att["title"])
                        attributeXML.set("type", att["type"])
                        if att["defaultValue"]:
                            sub_element = SubElement(attributeXML, "default")
                            sub_element.text = att["defaultValue"]
                    allAttributesXML.append(attributesXML)
        return allAttributesXML

    @staticmethod
    def getAttributesXML(atts):
        """Get XML attValues for an element (Node or Edge)
        by passing an attribute values list (stored in Nodes and Edges)"""
        if len(atts) > 0:
            attValuesXML = Element("attvalues")
            for att in atts:
                attValueXML = SubElement(attValuesXML, "attvalue")
                attValueXML.set("for", str(att["id"]))
                attValueXML.set("value", att["value"])
                if "start" in att.keys() and not att["start"] == "":
                    attValueXML.set("start" if not "startopen" in att.keys() or not att[
                                    "startopen"] else "startopen", att["start"])
                if "end" in att.keys() and not att["end"] == "":
                    attValueXML.set("end" if not "endopen" in att.keys() or not att[
                                    "endopen"] else "endopen", att["end"])
            return attValuesXML
        else:
            return None
    ...
    def importAttributesXML(self, attributes_xml):
        ...
        for child in attributes_xml:
            ...
            if tag == "attribute":
                ...
                self.declareAttribute(
                    attr_class, type, default, title, mode, id)
        ...
        for attvalues in attvalues_xml:
            for child in attvalues:
                ...
                if tag == "attvalue":
                    ...
                    atts.append(self.makeAttributeInstance(
...
class Spells(list):
    ...
    def getXML(self):
        ...
        spellsXML = Element("spells")
        for spell in self:
            spellXML = SubElement(spellsXML, "spell")
...
class Node:
    ...
    def addAttribute(self, id, value, start="", end="", startopen=False, endopen=False):
        self._attributes.append(self._graph.attributes.makeAttributeInstance(
            "node", id, value, start, end, startopen, endopen))

    def getXML(self):
        # return lxml etree element
        try:
            nodeXML = Element("node", id=str(self.id), label=self.label)
            if not self.start == "":
                nodeXML.set(
                    "start" if not self.startopen else "startopen", self.start)
            if not self.end == "":
    ...
    def importXML(nodes_xml, graph_obj):
        ...
        for child in nodes_xml:
            ...
            if tag == "node":
                ...
                node_obj = graph_obj.addNode(
                    id=id, label=label, start=start, end=end, startopen=startopen, endopen=endopen, pid=pid, r=r, g=g, b=b, spells=spells)
...
class Edge:
    ...
    def getXML(self):
        ...
        try:
            edgeXML = Element(
                "edge", id=str(self.id), source=str(self._source), target=str(self._target))
            if not self.start == "":
                edgeXML.set(
                    "start" if not self.startopen else "startopen", self.start)
            if not self.end == "":
...
class GexfImport:
    ...
    # deprecated : import XML codes are now included to the Gexf, Graph,
    # Attribute, Node, Edge classes
    ...
    def extract_node_attvalue(self, attvalue_xml):
        ...
        for attr in attvalue_xml.attrib:
            ...
            if attr == "endopen":
                ...
                endopen = True
        self.node_obj.addAttribute(
            id=id, value=value, start=start, end=end, startopen=startopen, endopen=endopen)
    ...
    def extract_edge(self, edge_xml):
        ...
        self.edge_obj = self.graph_obj.addEdge(id=id, source=source, target=target, weight=weight, start=start,
                                               end=end, startopen=startopen, endopen=endopen, label=label, r=r, g=g, b=b, spells=spells)
